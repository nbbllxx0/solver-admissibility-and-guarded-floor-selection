"""Figure generation for the restructured (v2) solver-admissibility manuscript.

The v2 figure set replaces the ten-figure v1 set with six main-text figures and
four supplementary figures. All figures are regenerated from the same tracked
CSV artifacts used by the v1 script; no numbers are re-derived by hand.

Semantic colour convention used everywhere in the set:
    teal    -> original floor preserved (keep)
    blue    -> rescued at 1e-3
    orange  -> rescued at 1e-2
...
C = {
    ...
    "r3": "#2F6F9F",        # blue   - rescued at 1e-3
    "r2": "#C56B2C",        # orange - rescued at 1e-2
...
# --------------------------------------------------------------------------
def fig1_policy_and_guard() -> None:
    ...
    _stage(ax, 1, 22, R2, BH, "rescue ladder", "$10^{-3} \\rightarrow 10^{-2}$", C["r2"])
    ...
    dx, dy, dhw, dhh = 80.0, R1 + BH / 2, 11.0, 7.0
    _diamond(ax, dx, dy, dhw, dhh, "keep or\nraise?")
    ...
    ax.text(58, CH_MID + 1.0, "keep: try $\\rho_0$ first", fontsize=7.8, color=C["keep"],
            ha="center", va="bottom")

    # raise branch: diamond -> ladder
    _elbow(ax, [(dx, dy + dhh), (dx, CH_TOP), (7.5, CH_TOP), (7.5, R2 + BH)], C["r2"])
    ax.text(43, CH_TOP + 1.2, "raise: enter the ladder", fontsize=7.8, color=C["r2"],
            ha="center", va="bottom")
    ...
    for yi, r in zip(y, rows):
        ...
        ax.text(r["true"] * 1.35, yi + 0.16, f"${r['true']/1e-6:.1f}\\tau$",
    ...
    ax.set_xlabel("relative residual at the accepted iterate")
    ...
    _save(fig, "fig1_policy_and_guard")


# --------------------------------------------------------------------------
# Figure 2: silent failure inside the optimization loop
# --------------------------------------------------------------------------
def fig2_silent_failure_in_loop() -> None:
    ...
    for row, (title, fixed, gd) in enumerate(blocks):
        ...
        if len(capped):
            ax.axvspan(float(capped.iteration.min()) - 0.5, 40.5, color=C["bad"], alpha=0.07,
                       zorder=0)
        ...
        ax.text(cont_iter - 0.8, lo * 1.15, "continuation\n$p:1.5\\!\\to\\!3.5$",
                fontsize=7.4, color=C["gray"], va="bottom", ha="right")
        if len(capped):
            ax.text(40.2, lo * 1.15, f"{len(capped)} of 40 solves\nat the budget cap",
                    fontsize=7.4, color=C["bad"], va="bottom", ha="right")
    ...
    fig.tight_layout()
    _save(fig, "fig2_silent_failure_in_loop")
...
def fig3_heldout_decision_map() -> None:
    d = _heldout_frame()
    ...
    for geom, marker, name in [("cantilever_gpu_medium", "o", "cantilever"),
        ...
        for is_keep, color, lab in [(1, C["keep"], "true keep"), (0, C["r2"], "true raise")]:
            m = (d.preset == geom) & (d.true_keep_original_floor == is_keep)
            if not m.any():      # e.g. every bridge state is a true raise
                continue
    ...
    ax.scatter(fk.probe_r50, fk.probe_r100_over_r50, marker="o", s=140, facecolor="none",
               edgecolor=C["bad"], lw=1.6, zorder=4, label="false keep (guard recovered)")
    ax.set_xscale("log")
    ...
    ax.text(1.5e-2, 1.24, "high-$r_{50}$ raise", fontsize=7.6, color=C["r2"], va="top")
    ax.text(8e-4, 0.70, "plateau raise", fontsize=7.2, color=C["r3"], va="top", ha="center")
    ax.annotate("probe converged before iteration 50:\n$r_{100}=r_{50}$, no trigger fires",
                xy=(4e-7, 0.985), xytext=(4.5e-8, 0.50), fontsize=7.0, color=C["keep"],
                arrowprops=dict(arrowstyle="-", color=C["keep"], lw=0.9))
    ...
    for i in range(2):
        for j in range(2):
            ax.text(j, i, f"{int(mat[i, j])}", ha="center", va="center", fontsize=17,
                    ...
                    fontweight="bold")
    ax.set_xticks([0, 1], ["predict raise", "predict keep"])
    ax.set_yticks([0, 1], ["true raise", "true keep"])
    ax.set_title("Rule against the full-budget audit")
    ax.text(0.5, -0.16,
            "sensitivity $74/78=94.9\\%$, specificity $24/24=100\\%$;\n"
            "the guard recovers all four false keeps, so the\n"
            "guarded policy converges on 102/102 states",
            transform=ax.transAxes, ha="center", va="top", fontsize=7.4)
    for s in ax.spines.values():
        s.set_visible(False)
    _save(fig, "fig3_heldout_decision_map")


# --------------------------------------------------------------------------
# Figure 4: what the policy costs
# --------------------------------------------------------------------------
def fig4_policy_cost() -> None:
    ...
    ax.set_xticks(x, [names[p] for p in order], fontsize=7.2)
    ...
    ax.bar(xi, failed, bottom=sel + probe, color=C["bad"], label="failed ladder attempt")
    ...
    labels = {"cantilever random prospective": "prospective cantilever",
    ...
    for gi, (category, group) in enumerate(cases.groupby("category", sort=False)):
        ...
        ax.text(float(idx.mean()), offset, f"{labels.get(category, category)} ({len(idx)})",
                ...
                color=C["gray"])
    ax.text(0.03, 0.70, f"all {int(failed.sum())} failed-ladder iterations\n"
...
def fig5_operator_perturbation() -> None:
    ...
    _box_points(ax, groups, [C["r3"], C["r2"]],
                "$|\\Delta C|/C$ (%)", "Compliance, 24 random keeps")
    ...
    if opt_path.exists():
        ...
        for i, (_, g) in enumerate(groups, start=1):
            ax.text(i, max(0.02, g.max() * 1.35), f"mean {g.mean():.3f}",
                    ha="center", fontsize=8.0)
        for mean_val, col, lab in [
            (pert[pert.rho_min == 1e-3].rel_dc_l2_solid.mean(), C["r3"], "random mean, $10^{-3}$"),
            (pert[pert.rho_min == 1e-2].rel_dc_l2_solid.mean(), C["r2"], "random mean, $10^{-2}$"),
        ]:
            ax.axhline(mean_val, color=col, ls="--", lw=1.0, alpha=0.8)
        ax.text(2.42, pert[pert.rho_min == 1e-2].rel_dc_l2_solid.mean(),
                "random-state\nmeans (panel b)",
                fontsize=6.8, va="center", ha="right", color=C["ink"])
        ax.set_yscale("log")
    ...
    fig.tight_layout()
    _save(fig, "fig5_operator_perturbation")


# --------------------------------------------------------------------------
# Figure 6: rescue histories and the stack-mechanism ablation
# --------------------------------------------------------------------------
def fig6_rescue_and_mechanism() -> None:
    bridge = pd.read_csv(
    ...
    axes[0].legend(fontsize=7.2, loc="lower right", bbox_to_anchor=(1.0, 0.02),
                   frameon=True, framealpha=0.95, edgecolor="none", facecolor="white",
                   borderpad=0.35)

    ax = axes[3]
    _panel(ax, next(letters), x=-0.24)
    labels = {"canonical": "reported stack", "levels3": "3-level hierarchy",
              "jacobi_smoother": "Jacobi smoother", "w_cycle": "W-cycle",
              "tol1em5": "tolerance $10^{-5}$", "tol1em7": "tolerance $10^{-7}$",
              "no_root_correction": "no root correction"}
    order = ["canonical", "levels3", "jacobi_smoother", "w_cycle", "tol1em5", "tol1em7",
             "no_root_correction"]
    m = mech.set_index("stack_variant").loc[order]
    y = np.arange(len(order))[::-1]
    ax.barh(y, m.passes, color=C["keep"], label="converged")
    ...
    ax.set_title("Stack ablation", fontsize=8.8)
    ax.grid(axis="x", alpha=0.12)
    _save(fig, "fig6_rescue_and_mechanism")
...
def figS4_policy_sensitivity() -> None:
    ...
    for letter, ax, (df, keycol, title, xlabel) in zip(
        ...
        ax.bar(x, r3, bottom=kept, width=0.62, color=C["r3"], label="rescued at $10^{-3}$")
        ax.bar(x, r2, bottom=np.array(kept) + np.array(r3), width=0.62, color=C["r2"],
               label="rescued at $10^{-2}$")
...
def figS2_threshold_sensitivity() -> None:
    ...
    for letter, ax, col, cmap, title in [
        ("a", axes[0], "false_keep", "Reds", "Unsafe missed raises"),
        ("b", axes[1], "false_raise", "Blues", "Conservative false raises"),
    ]:
        m = _grid(col)
        ...
        ax.annotate("reported rule", xy=(jh + 0.5, jr + 0.5), xytext=(jh + 1.9, jr + 1.5),
...
def figS3_optimized_gallery() -> None:
    ...
    for ax, (name, shape, title, cam, zoom, pscale) in zip(axes, cases):
        ...
        ax.set_axis_off()
        floor = "raised to $10^{-2}$" if raised else "kept $10^{-12}$"
...
def main() -> None:
    ...
    _style()
    fig1_policy_and_guard()
    fig2_silent_failure_in_loop()
    fig3_heldout_decision_map()
    fig4_policy_cost()
    fig5_operator_perturbation()
    fig6_rescue_and_mechanism()
